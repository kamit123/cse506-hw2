...
/* name and values of the xattr used for malware detection */
#define AMFS_XATTR_SSN "security.state"
#define AMFS_XATTR_SSG "good\0"
#define AMFS_XATTR_SSB "bad\0"
#define AMFS_XATTR_SSV_MAX 5
#define AMFS_XATTR_VER "security.version"
...
/* amfs virus pattern in memory read from pattdb */
struct amfs_virus_pattern {
	char *pattern;
...
static inline struct super_block *amfs_lower_super(const struct super_block
						   *sb)
{
...
static inline void amfs_set_pattdb_src(struct super_block *sb, char *src)
{
	AMFS_SB(sb)->pattdb_src = kmalloc(strlen(src), GFP_KERNEL);
	strcpy(AMFS_SB(sb)->pattdb_src, src);
}
...
static inline int amfs_is_bad_dentry(struct dentry *dentry,
				     struct super_block *sb)
{
	...
	if (vfs_getxattr(dentry, AMFS_XATTR_SSN, state, AMFS_XATTR_SSV_MAX)
	    > 0
	    && vfs_getxattr(dentry, AMFS_XATTR_VER, &version,
			    sizeof(version)) > 0) {
		if (strcmp(state, AMFS_XATTR_SSB) == 0
		    && AMFS_SB(sb)->version == version) {
			rc = 1;
		}
	}

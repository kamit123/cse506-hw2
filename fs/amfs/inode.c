			...
			const char *symname)
{
	...
	err =
	    vfs_symlink(lower_parent_dentry->d_inode, lower_dentry,
			symname);
	if (err)
		goto out;
	err = amfs_interpose(dentry, dir->i_sb, &lower_path);
...
static int amfs_mkdir(struct inode *dir, struct dentry *dentry,
		      umode_t mode)
{
	int err;
	struct dentry *lower_dentry;
...
static int amfs_mknod(struct inode *dir, struct dentry *dentry,
		      umode_t mode, dev_t dev)
{
	...
	err =
	    vfs_mknod(lower_parent_dentry->d_inode, lower_dentry, mode,
		      dev);
	if (err)
		goto out;
...
static int amfs_readlink(struct dentry *dentry, char __user * buf,
			 int bufsiz)
{
	int err;
	struct dentry *lower_dentry;
...
static int amfs_removexattr(struct dentry *dentry, const char *name)
{
	int err;
	struct dentry *lower_dentry;
	struct path lower_path;
	...
	amfs_get_lower_path(dentry, &lower_path);
	lower_dentry = lower_path.dentry;
	if (strcasecmp(name, AMFS_XATTR_SSN) == 0
	    || strcasecmp(name, AMFS_XATTR_VER) == 0) {
		printk(KERN_ERR
		       "not allowed to remove state and version XATTRs\n");
		err = -EINVAL;
		goto out;
	}
